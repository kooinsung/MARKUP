...
$(function() {            
	...
	//faq
	$('.article_tit.first').click(function(e) {  
		if($(this).next('tr').length > 0){
			...
			if (subNav.is(':visible')){
				subNav.hide()
					...
					$('.article_conts.second').hide();
				$('.article_conts.first').hide();
				subNav.show();
			}
		};
	});
	$('.article_tit.second').click(function(e) {  
		e.preventDefault();
		if($(this).next('tr').length > 0){
			var subNav = $(this).next('tr');
			if (subNav.is(':visible')){
				...
				$('.article_conts.second').hide();
				$('.article_tit.second.active').removeClass('active');
				$(this).addClass('active');
				subNav.show();
			}
		};
