...
$(function() {            
	...
	//val»èÁ¦ ¹öÆ° show/hide
	$(document).on('focus', '.review_input input', function(){
		$(this).val('');
	});
	$(document).on('keyup', '.review_input input', function(){
	...
	//º°Á¡
	 $.fn.raty.defaults.path = '../../img/mypage/';
	 $('.default').raty();

	//Ç³¼±
	baloonUp();
	function baloonUp(){
		$('.sale_baloon').animate({'top':'-20px'}, 1000, null, baloonDown);
	}
	function baloonDown(){
		$('.sale_baloon').animate({'top':'-10px'}, 700, null, baloonUp);
	}

	//»óÇ° ¿À¹ö
	$('.detail_tab_conts .togather_prd .item, .item_a .best_list .four > li, .item_a .item_srch_list .four > li').on('mouseenter', function () {
		$(this).find('.btns').stop().fadeIn('fast');
	}).mouseleave(function(){
		$(this).find('.btns').stop().hide();
	});

	$('.detail_tab_conts .plan_prd li').on('mouseenter', function () {
		$(this).find('.plan_prd_view').stop().fadeIn('fast');
	}).mouseleave(function(){
		$(this).find('.plan_prd_view').stop().hide();
	});

	//¸®ºä
	$('.btn_view_toggle').on("click",function(e) {
		e.preventDefault();
		if($(this).data("show")=="no") {
			//´ÝÈû
			$(this).removeClass('active');
			$(this).parent().find('.review_txt').removeClass('open');
			$(this).parent().find('.btns').hide();
			$(this).data("show","yes");
		}else{
			//¿­¸²
			$(this).addClass('active');
			$(this).parent().find('.review_txt').addClass('open');
			$(this).parent().find('.btns').show();
			$(this).data("show","no");
		}
	});

	$('.btn_view_layer').on("click mouseenter",function(e) {
		e.preventDefault();
		if($(this).data("show")=="no") {
			//´ÝÈû
			$(this).siblings('.common_layer').stop().fadeOut();
			$(this).data("show","yes");
		}else{
			//¿­¸²
			$('.common_layer ').hide();
			$(this).siblings('.common_layer').stop().fadeIn();
			$('.common_layer').on('mouseleave', function () {
				$(this).stop().fadeOut();
				$('.btn_view_layer').data("show","yes");
			});
			$(this).data("show","no");
		}
	});

	$('.item_a .sort_list button').on('click', function () {
		$('.item_a .sort_list button').removeClass('active');
		$(this).addClass('active');
	});



	$('.item_a .sort_brand_list .more').on("click",function(e) {
		e.preventDefault();
		if($(this).data("show")=="no") {
			//´ÝÈû
			$(this).removeClass('close').text('...more');
			$(this).parent().find('ul').css('height','24px');
			$(this).data("show","yes");
		}else{
			//¿­¸²
	...
	$('.view_type li button').on('click', function () {
		...
		$(this).addClass('active');
		$('.item_srch_list ul').removeAttr('class');
		if(num == "0"){
			$('.item_srch_list ul').addClass('two');
		}else if(num == "1"){
			$('.item_srch_list ul').addClass('three');
		}else if(num == "2"){
			$('.item_srch_list ul').addClass('four');
		}
	});
